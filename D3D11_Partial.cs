...
namespace D3D11
{
    public class D3D11Tools
    {
        ...
        public static ID3D11ShaderResourceView? CreateTextureFromFile(string sFilePath, IWICImagingFactory pWICImagingFactory, ID3D11Device pD3D11Device)
        {
            HRESULT hr = HRESULT.S_OK;            
            hr = pWICImagingFactory.CreateDecoderFromFilename(sFilePath, Guid.Empty, unchecked((int)GENERIC_READ),WICDecodeOptions.WICDecodeMetadataCacheOnLoad,
                out IWICBitmapDecoder pDecoder); 
            if (SUCCEEDED(hr))
            {
                hr = pDecoder.GetFrame(0, out IWICBitmapFrameDecode pFrameDecode);
                if (SUCCEEDED(hr))
                {
                    hr = pFrameDecode.GetSize(out uint nWidth, out uint nHeight);
                    if (SUCCEEDED(hr))
                    {
                        hr = pWICImagingFactory.CreateFormatConverter(out IWICFormatConverter pFormatConverter);
                        if (SUCCEEDED(hr))
                        {
                            hr = pFormatConverter.Initialize(
                            pFrameDecode,
                            WICTools.GUID_WICPixelFormat32bppRGBA,
                            ...
                            if (SUCCEEDED(hr))
                            {
                                int nStride = (int)(nWidth * 4);
                                byte[] pixels = new byte[nStride * nHeight];

                                hr = pFormatConverter.CopyPixels(IntPtr.Zero, (uint)nStride, (uint)pixels.Length, pixels);
                                if (SUCCEEDED(hr))
                                {
                                    ...
                                    unsafe
                                    {
                                        fixed (byte* pPixels = pixels)
                                        {
                                            ...
                                            {
                                                pSysMem = (IntPtr)pPixels,
                                                SysMemPitch = (uint)nStride
                                            };
                                            IntPtr pSubresource = Marshal.AllocHGlobal(Marshal.SizeOf<D3D11_SUBRESOURCE_DATA>());
                                            Marshal.StructureToPtr(subresource, pSubresource, false);
                                            hr = pD3D11Device.CreateTexture2D(ref desc, pSubresource, out ID3D11Texture2D pTexture);
                                            if (SUCCEEDED(hr))
                                            { 
                                                ...
                                                hr = pD3D11Device.CreateShaderResourceView(pTexture, ref srvDesc, out ID3D11ShaderResourceView srv);
                                                if (SUCCEEDED(hr))
                                                {
                                                    Marshal.FreeHGlobal(pSubresource);
    ...
    [StructLayout(LayoutKind.Sequential)]
    public unsafe struct D3D11_DEPTH_STENCIL_VIEW_DESC
    {
        public DXGI_FORMAT Format;
    ...
    [StructLayout(LayoutKind.Sequential)]
    public unsafe struct D3D11_UNORDERED_ACCESS_VIEW_DESC
    {
        public DXGI_FORMAT Format;
    ...
    public interface ID3D11Device
    {
        ...
        //HRESULT CreateRenderTargetView(ID3D11Resource pResource, ref D3D11_RENDER_TARGET_VIEW_DESC pDesc, out ID3D11RenderTargetView ppRTView);
        [PreserveSig]
        ...
        [PreserveSig]
        HRESULT CreateGeometryShaderWithStreamOutput(ID3D11Resource pResource, ref D3D11_UNORDERED_ACCESS_VIEW_DESC pDesc, out ID3D11UnorderedAccessView ppUAView);
        [PreserveSig]
    ...
    public interface ID3D11VideoDevice
    {
        ...
        [PreserveSig]
        HRESULT CreateVideoDecoderOutputView(ID3D11Resource pResource, ref D3D11_VIDEO_DECODER_OUTPUT_VIEW_DESC pDesc, out ID3D11VideoDecoderOutputView ppVDOVView);
        [PreserveSig]
    ...
    public interface ID3D11Device1 : ID3D11Device
    {
        ...
        [PreserveSig]
        new HRESULT CreateShaderResourceView(ID3D11Resource pResource, ref D3D11_SHADER_RESOURCE_VIEW_DESC pDesc, out ID3D11ShaderResourceView ppSRView);
        [PreserveSig]
        new HRESULT CreateUnorderedAccessView(ID3D11Resource pResource, ref D3D11_UNORDERED_ACCESS_VIEW_DESC pDesc, out ID3D11UnorderedAccessView ppUAView);
        //new HRESULT CreateRenderTargetView(ID3D11Resource pResource, ref D3D11_RENDER_TARGET_VIEW_DESC pDesc, out ID3D11RenderTargetView ppRTView);
        [PreserveSig]
        ...
        [PreserveSig]
        new HRESULT CreateGeometryShaderWithStreamOutput(ID3D11Resource pResource, ref D3D11_UNORDERED_ACCESS_VIEW_DESC pDesc, out ID3D11UnorderedAccessView ppUAView);
        [PreserveSig]
